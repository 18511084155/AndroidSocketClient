# AndroidSocketClient 简介
一个Android轻量级Socket通讯框架

### <font id="1">AndroidSocketClient简介</font>
<font size=2>Android Socket是一款基于阻塞式传统Socket的一款Socket客户端整体解决方案.您可以使用它进行简单的基于Tcp协议的Socket通讯,当然,也可以进行大数据量复杂的Socket通讯,
支持单工,双工通讯.</font>


### <font id="2">Maven配置</font>
##### <font id="2.1">自动导入(推荐)</font>
* <font size=2>AndroidSocketClient 支持 JCenter 直接导入</font>
...
### <font id="4">混淆配置</font>
* <font size=2>请避免混淆AndroidSocketClient，在Proguard混淆文件中增加以下配置：</font>
...
-keep class com.woodys.libsocket.sdk.AndroidSocketClientOptions$* {
    *;
}
-keep class com.woodys.libsocket.sdk.AndroidSocketClientSSLConfig$* {
    *;
}

```

### <font id="5">AndroidSocketClient初始化</font>
* <font size=2>将以下代码复制到项目Application类onCreate()中，AndroidSocketClient会为自动检测环境并完成配置：</font>
...
public class MyApplication extends Application {
	...
	public void onCreate() {
		...
		AndroidSocketClient.initialize(this);
		//如果需要开启Socket调试日志,请配置
		//AndroidSocketClient.initialize(this,true);
	}
}
```
### <font id="6">调用演示</font>
#### 测试服务器
<font size=2> 该服务器是专门为初学者调试 AndroidSocketClient 库部属的一台测试服务器,初学者可以将项目中的 app 安装到手机上,点击 `Connect` 按钮即可,该服务器仅为熟悉通讯方式和解析方式使用.该服务器不支持心跳返回,不能作为商用服务器.服务器代码在 `SocketServerDemo` 文件夹中,请注意参考阅读. </font>
...
//调用AndroidSocketClient,开启这次连接的通道,调用通道的连接方法进行连接.
AndroidSocketClient.open(info).connect();
```
##### <font id="6.2">有回调的长连接</font>
* <font size=2> 注册该通道的监听器,每个 Connection 通道中的监听器互相隔离,因此如果一个项目连接了多个 Socket 连接需要在每个 Connection 注册自己的连接监听器,连接监听器是该 AndroidSocketClient 与用户交互的唯一途径</font>

```java
//连接参数设置(IP,端口号),这也是一个连接的唯一标识,不同连接,该参数中的两个值至少有其一不一样
ConnectionInfo info = new ConnectionInfo("104.238.184.237", 8080);
//调用AndroidSocketClient,开启这次连接的通道,拿到通道Manager
IConnectionManager manager = AndroidSocketClient.open(info);
...
* <font size=2> 获得 AndroidSocketClientOptions 的行为属于比较高级的 AndroidSocketClient 调用方法,每个 Connection 将会对应一个 AndroidSocketClientOptions,如果第一次调用 Open 时未指定 AndroidSocketClientOptions,AndroidSocketClient将会使用默认的配置对象,默认配置请见文档下方的高级调用说明</font>
...
IConnectionManager manager = AndroidSocketClient.open(info);
//获得当前连接通道的参配对象
AndroidSocketClientOptions options= manager.getOption();
//基于当前参配对象构建一个参配建造者类
AndroidSocketClientOptions.Builder builder = new AndroidSocketClientOptions.Builder(options);
//修改参配设置(其他参配请参阅类文档)
...
public void onSocketConnectionSuccess(Context context, ConnectionInfo info, String action) {
     //连接成功其他操作...
     //链式编程调用
     AndroidSocketClient.open(info)
     	.send(new TestSendData());
     
     //此处也可将ConnectManager保存成成员变量使用.
     mManager = AndroidSocketClient.open(info);
     if(mManager != null){
...
```java
//设置自定义解析头
AndroidSocketClientOptions.Builder okOptionsBuilder = new AndroidSocketClientOptions.Builder(mOkOptions);
...
```
### <font id="7">AndroidSocketClient参配选项及回调说明</font>
* AndroidSocketClientOptions
